...
function privacyPage() {
  return layout(
    'Privacy Policy',
    '<main class="legal"><div class="eyebrow">Privacy</div><h1>Less data.<br><em>More clarity.</em></h1><p>Last updated: September 25, 2026</p><h2>What TikClip processes</h2><p>When you message the TikClip Telegram bot, we process the Telegram message, your chat identifier, and the TikTok URL you provide only to retrieve, upload, and send the requested video.</p><h2>What we store</h2><p>TikClip does not intentionally store your messages, TikTok links, downloaded videos, or media files. The service may keep short-lived in-memory request state while a download or publishing operation is being processed.</p><h2>Service providers</h2><p>Video requests are sent to the TikWM API to resolve a downloadable video URL. Telegram processes messages and delivers bot responses. If you choose to connect an account, TikTok or Google processes OAuth data and the selected platform receives the video. These services have their own policies.</p><h2>Contact</h2><p>For privacy questions, contact the project owner through the GitHub repository linked from the project deployment.</p></main>',
  );
}
...
function termsPage() {
  return layout(
    'Terms of Service',
    '<main class="legal"><div class="eyebrow">Terms</div><h1>Use it<br><em>responsibly.</em></h1><p>Last updated: September 25, 2026</p><h2>Acceptable use</h2><p>You may use TikClip only for lawful purposes and only to download content you have permission to use. Do not use the service to infringe copyrights, evade access controls, or distribute harmful content.</p><h2>Availability</h2><p>TikClip is provided as-is. Video retrieval and publishing depend on Telegram, TikWM, TikTok, Google, network availability, and the source video. We do not guarantee that every link will work or remain available.</p><h2>Third-party services</h2><p>TikClip is independent and is not affiliated with TikTok, YouTube, or Google. You are responsible for following each platform\'s terms, copyright rules, and any applicable laws.</p><h2>Changes</h2><p>These terms may change as the service evolves. Continued use after an update means you accept the revised terms.</p></main>',
  );
}
